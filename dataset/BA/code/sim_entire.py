...
m = args.m # random graph probability to connect
...
graph_name = gpath + '%s_N%s_m%s_g%s.edgelist' % (graph_type,N,m,graph_id)
...
if sim_type == 'SIR':
    sim_file = '%s_Rzero%s_gamma%s_ls%s_nf%s_N%s_m%s_g%s_entire.pickle' %\
    (sim_type,R0,gamma,len_seq,num_frames,N,m,graph_id)
elif sim_type == 'SEIR':
    sim_file = '%s_Rzero%s_gamma%s_alpha%s_ls%s_nf%s_N%s_m%s_g%s_entire.pickle' %\
    (sim_type,R0,gamma,alpha,len_seq,num_frames,N,m,graph_id)
else:

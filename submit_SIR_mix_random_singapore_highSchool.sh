...
gt=singapore_highSchool     # random graph type, ER, BA, BA-Tree, RGG
...
for (( run=0; run < 5; run++ )) do
	python main_SIR_mix_random_singapore_highSchool.py --valid ${valid} --gt ${gt} --n_node ${N} --n_frame ${nf} --batch_size ${bs} --epoch ${ep} --ks ${ks} --kt ${kt} --sconv ${sc} --save ${save} --graph $g --seq ${s1} --start ${skip} --end ${end}
	python test_SIR_mix_random_singapore_highSchool.py --valid ${valid} --gt ${gt} --n_node ${N} --n_frame ${nf} --batch_size ${bs} --epoch ${ep} --ks ${ks} --kt ${kt} --sconv ${sc} --save ${save} --graph $g --seq ${s2} --start ${skip} --end ${end} | tail -n 1 >> ${output}
